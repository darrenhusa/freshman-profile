<div class="item-container ml-5 mt-5">

  <h1 class="text-xl font-bold">10</h1>
  <h2>Top Ten Majors for FTFT F1 Students</h2>

  {{-- <img src="/images/top-majors-data.png" alt=""> --}}

  <div class="table-container ml-10">
    <table class="table-auto">
      <thead>
        <tr>
          <th class="px-4 py-2"></th>
          <th class="px-4 py-2">Number (N)</th>
          <th class="px-4 py-2">Percentage (%)</th>
          <th class="px-4 py-2">Current Rank</th>
        </tr>
      </thead>
      <tbody class="text-right">
        <tr>
          <td class="border px-4 py-2 text-left">Business Managment</td>
          <td class="border px-4 py-2">27</td>
          <td class="border px-4 py-2">{{ round(100*27/157, 0) }}%</td>
          <td class="border px-4 py-2">1</td>
        </tr>
        <tr class="bg-gray-200">
          <td class="border px-4 py-2 text-left">Criminal Justice</td>
          <td class="border px-4 py-2">25</td>
          <td class="border px-4 py-2">{{ round(100*25/157, 0) }}%</td>
          <td class="border px-4 py-2">2</td>
        </tr>
        <tr>
          <td class="border px-4 py-2 text-left">Biomedical Science</td>
          <td class="border px-4 py-2">18</td>
          <td class="border px-4 py-2">{{ round(100*18/157, 0) }}%</td>
          <td class="border px-4 py-2">3</td>
        </tr>
        <tr class="bg-gray-200">
          <td class="border px-4 py-2 text-left">General Studies</td>
          <td class="border px-4 py-2">18</td>
          <td class="border px-4 py-2">{{ round(100*18/157, 0) }}%</td>
          <td class="border px-4 py-2">3</td>
        </tr>
        <tr>
          <td class="border px-4 py-2 text-left">Elementary Education</td>
          <td class="border px-4 py-2">11</td>
          <td class="border px-4 py-2">{{ round(100*11/157, 0) }}%</td>
          <td class="border px-4 py-2">5</td>
        </tr>
        <tr class="bg-gray-200">
          <td class="border px-4 py-2 text-left">Digital & Studio Arts</td>
          <td class="border px-4 py-2">10</td>
          <td class="border px-4 py-2">{{ round(100*10/157, 0) }}%</td>
          <td class="border px-4 py-2">6</td>
        </tr>
        <tr>
          <td class="border px-4 py-2 text-left">Psychology</td>
          <td class="border px-4 py-2">10</td>
          <td class="border px-4 py-2">{{ round(100*10/157, 0) }}%</td>
          <td class="border px-4 py-2">6</td>
        </tr>
        <tr class="bg-gray-200">
          <td class="border px-4 py-2 text-left">Kinesiology</td>
          <td class="border px-4 py-2">9</td>
          <td class="border px-4 py-2">{{ round(100*9/157, 0) }}%</td>
          <td class="border px-4 py-2">8</td>
        </tr>
        <tr>
          <td class="border px-4 py-2 text-left">Health Science</td>
          <td class="border px-4 py-2">7</td>
          <td class="border px-4 py-2">{{ round(100*7/157, 0) }}%</td>
          <td class="border px-4 py-2">9</td>
        </tr>
        <tr class="bg-gray-200">
          <td class="border px-4 py-2 text-left">Human Services</td>
          <td class="border px-4 py-2">7</td>
          <td class="border px-4 py-2">{{ round(100*7/157, 0) }}%</td>
          <td class="border px-4 py-2">9</td>
        </tr>
        <tr class="bg-gray-200">
          <td class="border px-4 py-2">Total (from top 10)</td>
          <td class="border px-4 py-2">142</td>
          <td class="border px-4 py-2">{{ round(100*142/157, 0) }}%</td>
          <td class="border px-4 py-2">--</td>
        </tr>
        <tr class="bg-gray-200">
          <td class="border px-4 py-2">Grand Total (from all FTFT F1)</td>
          <td class="border px-4 py-2">157</td>
          <td class="border px-4 py-2">100%</td>
          <td class="border px-4 py-2">--</td>
        </tr>
      </tbody>
    </table>
    <div class="note mt-5">
      <h4 class="italic"><span class="underline">Note</span>: FTFT F1 = First-time, Full-time F1 students</h4>
    </div>

    <div class="source mt-5">
      <h4 class="italic"><span class="underline">Source</span>: Empower Student Records (SR)</h4>
    </div>

  </div>
</div>
